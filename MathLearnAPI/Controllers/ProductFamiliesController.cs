...
namespace MathLearnAPI.Controllers
{
    ...
    {
        private acquizdbContext _db;

        public ProductFamiliesController(acquizdbContext context)
        {
            _db = context;
        }
        ...
        /// <param name="productFamily"></param>
        [AcceptVerbs("PATCH")]
        public async Task<IActionResult> Patch(int key, Delta<ProductFamily> productFamily)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        /// </summary>
        public async Task<IActionResult> PostToProducts([FromODataUri] int key, Product product)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        public async Task<IActionResult> CreateProductsLink([FromODataUri] int key, [FromBody] Uri uri)
        {
            var family = await _db.ProductFamilies.FindAsync(key);
            if (family == null)
            {
                // return Content(HttpStatusCode.NotFound, ODataErrors.EntityNotFound("ProductFamilies"));
                return NotFound(ODataErrors.EntityNotFound("ProductFamilies"));
            }
            ...
            if (product == null)
            {
                ...
                return NotFound(ODataErrors.EntityNotFound("Products"));
            }
        ...
        public async Task<IActionResult> CreateSupplierLink([FromODataUri] int key, [FromBody] Uri uri)
        {
            var family = await _db.ProductFamilies.FindAsync(key);
            if (family == null)
            {
            ...
            if (supplier == null)
            {
                ...
                return NotFound(ODataErrors.EntityNotFound("Suppliers"));
            }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateProduct([FromODataUri] int key, ODataActionParameters parameters)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
